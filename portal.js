...
(function(){
  ...
  window.adminCatalogView = function(){
    ...
    return `<section class="panel" id="catalogAdminPanel"><div class="catalog-admin-head"><div><h2>🛠️ إدارة الموقع</h2><p>أضف المنتجات والعروض وعدّل إعدادات الموقع من هنا، من غير ما تلمس الكود.</p></div><span class="catalog-live">Firebase Live</span></div><div class="catalog-tabs"><button class="catalog-tab active" data-catalog-tab="products">المنتجات</button><button class="catalog-tab" data-catalog-tab="offers">العروض</button><button class="catalog-tab" data-catalog-tab="settings">إعدادات الموقع</button><button class="catalog-tab" data-catalog-tab="notifications">إشعارات العملاء</button></div><div class="catalog-pane active" data-catalog-pane="products"><form id="productAdminForm" class="catalog-form"><input id="productId" placeholder="كود اختياري — مثال: milk-1"><input id="productName" placeholder="اسم المنتج" required><select id="productCat"><option value="">اختار القسم</option>${categoryOptions()}</select><select id="productStoreId" required aria-label="المتجر"><option value="market-1">السوبر ماركت</option><option value="koshary-hind">كشري هند</option><option value="bakery-sixth">مخبز الجهيني</option><option value="pizza-rahma">بيتزا الرحمة</option><option value="pharmacy-1">صيدلية الجهيني</option></select><div class="catalog-grid2"><input id="productPrice" type="number" min="0" step="0.01" placeholder="السعر"><input id="productOldPrice" type="number" min="0" step="0.01" placeholder="السعر قبل الخصم"></div><div class="catalog-grid2"><input id="productEmoji" placeholder="إيموجي — مثال 🥛"><input id="productBadge" placeholder="شارة — الأكثر طلبًا / جديد"></div><input id="productImg" placeholder="رابط/مسار الصورة — أو ارفعها من هنا"><input id="productImageFile" type="file" accept="image/*"><label class="checkline"><input id="productReadyForOrder" type="checkbox"> السعر متأكد منه والمنتج متاح للطلب</label><small>سيب الاختيار مقفول لحد ما تتأكد من سعر المحل.</small><label class="checkline"><input id="productActive" type="checkbox" checked> ظاهر على الموقع</label><label class="checkline"><input id="productSoldOut" type="checkbox"> غير متوفر حاليًا</label><div class="catalog-actions"><button type="submit" class="action red">حفظ المنتج</button><button type="button" id="productReset" class="action">منتج جديد</button></div></form><div id="adminProductsList"><div class="empty">جارٍ تحميل المنتجات…</div></div></div><div class="catalog-pane" data-catalog-pane="offers"><form id="offerAdminForm" class="catalog-form"><input id="offerId" placeholder="كود اختياري — مثال: friday"><input id="offerTitle" placeholder="عنوان العرض" required><textarea id="offerDesc" rows="3" placeholder="وصف بسيط للعرض"></textarea><div class="catalog-grid2"><input id="offerTag" placeholder="الشارة — عرض اليوم"><input id="offerPriority" type="number" placeholder="الأولوية" value="0"></div><input id="offerExpiresAt" type="datetime-local"><label class="checkline"><input id="offerActive" type="checkbox" checked> ظاهر على الموقع</label><div class="catalog-actions"><button type="submit" class="action red">حفظ العرض</button><button type="button" id="offerReset" class="action">عرض جديد</button></div></form><div id="adminOffersList"><div class="empty">جارٍ تحميل العروض…</div></div></div><div class="catalog-pane" data-catalog-pane="settings"><form id="siteSettingsForm" class="catalog-form"><div class="catalog-grid2"><input id="siteArea" placeholder="منطقة التوصيل"><input id="siteAudience" placeholder="الجمهور"></div><input id="siteEta" placeholder="المدة — مثال حوالي 10 دقايق"><div class="catalog-grid2"><input id="siteDeliveryFee" type="number" min="0" placeholder="رسوم التوصيل"><input id="siteFreeDelivery" type="number" min="0" placeholder="مجاني فوق مبلغ"></div><div class="catalog-grid2"><input id="siteMinOrder" type="number" min="0" placeholder="الحد الأدنى للطلب"><label>يفتح الساعة<input id="siteOpenTime" type="time" value="06:00" required></label></div><label>يقفل الساعة<input id="siteCloseTime" type="time" value="14:00" required></label><label class="checkline"><input id="siteClosedToday" type="checkbox"> المتجر مقفول النهارده</label><input id="siteClosedTodayReason" maxlength="120" placeholder="سبب أو ملاحظة للعميل (اختياري)"><label>مفتاح Firebase Web Push العام<input id="sitePushVapidKey" maxlength="500" placeholder="يتضاف من إعدادات Firebase Cloud Messaging" dir="ltr"></label><small>المواعيد بتوقيت القاهرة. إعداد المفتاح مطلوب لتفعيل الإشعارات على الأجهزة.</small><div class="catalog-actions"><button type="submit" class="action red">حفظ الإعدادات</button></div></form></div><div class="catalog-pane" data-catalog-pane="notifications"><form id="broadcastForm" class="catalog-form"><h3>إشعار لكل العملاء المشتركين</h3><p>هيتم الإرسال للأجهزة اللي وافقت على الإشعارات فقط.</p><input id="broadcastTitle" maxlength="80" placeholder="عنوان الإشعار" required><textarea id="broadcastBody" maxlength="180" rows="3" placeholder="رسالة قصيرة للعملاء" required></textarea><div class="catalog-actions"><button type="submit" class="action red">إرسال الإشعار</button></div><small id="broadcastStatus">إرسال البث محتاج نشر Firebase Cloud Functions وإعداد مفتاح الإشعارات.</small></form></div></section>`;
  };
  async function adminProducts(){
    const local=Array.isArray(window.ACDLocalProducts)?window.ACDLocalProducts:[];
    const remote=await ACDCloud.adminListProducts();
    const merged=new Map(local.map(item=>[String(item.id),item]));
    remote.forEach(item=>merged.set(String(item.id),item));
    return [...merged.values()].sort((a,b)=>String(a.name||'').localeCompare(String(b.name||''),'ar'));
  }
  ...
  function bind(){
    ...
    $('#productAdminForm')?.addEventListener('submit',async e=>{e.preventDefault();try{let imageUrl=$('#productImg').value.trim();const file=$('#productImageFile')?.files?.[0];if(file){$('#productImg').value='جارٍ رفع الصورة…';imageUrl=await ACDCloud.adminUploadProductImage(file,$('#productId').value||$('#productName').value);$('#productImg').value=imageUrl;}await ACDCloud.adminSaveProduct({id:$('#productId').value,name:$('#productName').value,cat:$('#productCat').value,storeId:$('#productStoreId').value,price:$('#productPrice').value,oldPrice:$('#productOldPrice').value,emoji:$('#productEmoji').value,badge:$('#productBadge').value,img:imageUrl,active:$('#productActive').checked,soldOut:$('#productSoldOut').checked,comingSoon:!$('#productReadyForOrder').checked});e.target.reset();$('#productActive').checked=true;toast('تم حفظ المنتج ✅');await refreshLists();}catch(err){toast(ACDCloud.authError(err));}});
    ...
    $('#siteSettingsForm')?.addEventListener('submit',async e=>{e.preventDefault();try{await ACDCloud.adminSaveSiteSettings({area:$('#siteArea').value,audience:$('#siteAudience').value,eta:$('#siteEta').value,deliveryFee:$('#siteDeliveryFee').value,freeDelivery:$('#siteFreeDelivery').value,minOrder:$('#siteMinOrder').value,openTime:$('#siteOpenTime').value,closeTime:$('#siteCloseTime').value,closedToday:$('#siteClosedToday').checked,closedTodayReason:$('#siteClosedTodayReason').value,pushVapidKey:$('#sitePushVapidKey').value});toast('تم حفظ مواعيد المتجر وإعداداته ✅');}catch(err){toast(ACDCloud.authError(err));}});
    $('#broadcastForm')?.addEventListener('submit',async e=>{e.preventDefault();const b=e.target.querySelector('[type=submit]'),status=$('#broadcastStatus');if(!confirm('إرسال الإشعار لكل العملاء اللي وافقوا على استقبال الإشعارات؟'))return;try{b.disabled=true;b.textContent='جارٍ الإرسال…';const result=await ACDCloud.sendBroadcastNotification({title:$('#broadcastTitle').value,body:$('#broadcastBody').value});status.textContent=result.devices===0?'مفيش أجهزة مشتركة في الإشعارات لحد دلوقتي.':('تم الإرسال لـ '+result.sent+' جهاز. تعذر الإرسال إلى '+result.failed+' جهاز.');toast(result.sent?'تم إرسال الإشعار ✅':'مافيش أجهزة مفعّلة تستقبل الإشعار.');e.target.reset();}catch(err){status.textContent=ACDCloud.authError(err);toast(ACDCloud.authError(err));}finally{b.disabled=false;b.textContent='إرسال الإشعار';}});
    panel.addEventListener('click',async e=>{const pe=e.target.closest('[data-edit-product]');if(pe){const x=(await adminProducts()).find(v=>v.id===pe.dataset.editProduct);if(!x)return;$('#productId').value=x.id;$('#productName').value=x.name||'';$('#productCat').value=x.cat||'';$('#productStoreId').value=x.storeId||(['pain','cold','stomach','vitamins','firstaid','skin','hygiene'].includes(x.cat)?'pharmacy-1':'market-1');$('#productPrice').value=x.price||'';$('#productOldPrice').value=x.oldPrice||'';$('#productEmoji').value=x.emoji||'';$('#productBadge').value=x.badge||'';$('#productImg').value=x.img||'';$('#productActive').checked=x.active!==false;$('#productSoldOut').checked=!!x.soldOut;panel.querySelector('[data-catalog-tab="products"]').click();return;}const pd=e.target.closest('[data-delete-product]');if(pd){if(!confirm('إخفاء المنتج من الموقع؟'))return;try{await ACDCloud.adminDeleteProduct(pd.dataset.deleteProduct);toast('تم إخفاء المنتج ✅');await refreshLists();}catch(err){toast(ACDCloud.authError(err));}}const oe=e.target.closest('[data-edit-offer]');if(oe){const x=(await ACDCloud.adminListOffers()).find(v=>v.id===oe.dataset.editOffer);if(!x)return;$('#offerId').value=x.id;$('#offerTitle').value=x.title||'';$('#offerDesc').value=x.desc||'';$('#offerTag').value=x.tag||'';$('#offerPriority').value=x.priority||0;$('#offerExpiresAt').value=x.expiresAt||'';$('#offerActive').checked=x.active!==false;panel.querySelector('[data-catalog-tab="offers"]').click();return;}const od=e.target.closest('[data-delete-offer]');if(od){if(!confirm('إخفاء العرض من الموقع؟'))return;try{await ACDCloud.adminDeleteOffer(od.dataset.deleteOffer);toast('تم إخفاء العرض ✅');await refreshLists();}catch(err){toast(ACDCloud.authError(err));}}});
    ACDCloud.getSiteSettings().then(x=>{['area','audience','eta','deliveryFee','freeDelivery','minOrder','closedTodayReason','pushVapidKey'].forEach(k=>{const el=$('#site'+k.charAt(0).toUpperCase()+k.slice(1));if(el&&x[k]!==undefined)el.value=x[k];});const time=h=>String(Math.floor((Number(h)||0)%24)).padStart(2,'0')+':00';$('#siteOpenTime').value=x.openTime||time(6);$('#siteCloseTime').value=x.closeTime||time(14);const parts=new Intl.DateTimeFormat('en-GB',{timeZone:'Africa/Cairo',year:'numeric',month:'2-digit',day:'2-digit'}).formatToParts(new Date());const today=parts.find(v=>v.type==='year').value+'-'+parts.find(v=>v.type==='month').value+'-'+parts.find(v=>v.type==='day').value;$('#siteClosedToday').checked=x.closedToday===true&&x.closedTodayDate===today;}).catch(()=>{});
    refreshLists();
  }
