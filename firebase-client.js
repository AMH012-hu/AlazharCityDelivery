...
async function adminSaveSiteSettings(input) {
  ...
  const data = {
    ...
    openHour: Number(input?.openHour || 0),
    closeHour: Number(input?.closeHour || 0),
    updatedAt: serverTimestamp()
  };
...
async function createOrder(input) {
  ...
  const ref = await addDoc(collection(db, 'orders'), {
    ...
    status: 'new',
    riderId: null,
    storeId: input?.storeId || (input?.hasPharma ? 'pharmacy-1' : 'market-1'),
    rating: null,
    ratedAt: null,
    createdAt: serverTimestamp(),
    updatedAt: serverTimestamp()
...
window.ACDCloud = {
  ...
  adminListOffers, adminSaveOffer, adminDeleteOffer, getSiteSettings, adminSaveSiteSettings, adminUploadProductImage,
  watchOrders,
  signOut: signOutUser,
  user: () => currentUser,
